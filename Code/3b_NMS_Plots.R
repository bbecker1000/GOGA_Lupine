...
gg_all = ggplot(data = data.scores.all, aes(x = NMDS1, y = NMDS2)) + 
  ...
  geom_segment(aes(x = 0, y = 0, xend = NMDS1, yend = NMDS2),
               ...
               linewidth = 1, alpha = 1, colour = "grey30") +
  geom_text(data = en_coord_cont_all, aes(x = NMDS1, y = NMDS2), colour = "grey30",
            fontface = "bold", label = c("Yearly Rainfall", "Time Since Treatment")) +
...
gg_groups2 = ggplot(data = data.scores.group, aes(x = NMDS1, y = NMDS2)) + 
  geom_point(data = data.scores.group, aes(colour = Treatment, shape = Trt_Status), 
             size = 3, alpha = 0.5) + 
  stat_ellipse(data = data.scores.group, aes(x = NMDS1, y = NMDS2, colour  = Treatment, linetype = Trt_Status), 
               level = 0.8, alpha = 0.9, size = 1) +
  geom_segment(aes(x = 0, y = 0, xend = NMDS1, yend = NMDS2), 
