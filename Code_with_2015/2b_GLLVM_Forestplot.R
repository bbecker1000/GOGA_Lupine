...
rain.forestplot <- ggplot(glvvm.coef.plot.rain_2015, aes(Species, Estimate, color = nativity, shape = Species)) + 
  geom_pointrange(aes(ymin = Estimate - 2 * Estimate.sd, ymax = Estimate + 2 * Estimate.sd)) + 
  coord_flip() + 
  geom_hline(yintercept = 0, linetype = 2) + 
...
gllvm_plots_2015 <- afterburn + aftermech + rain.forestplot + plot_annotation(tag_levels = "a")
